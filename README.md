...
```bash
cd target-app/auth-service/database
docker compose up -d
# 初回のみ: スキーマ・シードデータ投入
...
```bash
cd target-app/app1-service-dashboard/database
docker compose up -d
# 初回のみ: スキーマ・シードデータ投入
...
```bash
cd target-app/app1-service-dashboard/backend/App1Backend
dotnet run
```
...
```bash
cd target-app/app1-service-dashboard/frontend
npm install   # 初回のみ
npm run dev
```
