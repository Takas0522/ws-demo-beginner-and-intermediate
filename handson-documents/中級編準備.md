...
## GitHub Copilot CLIのインストール(ここからMacのbashでもほぼ同様のコマンドで対応可能)
...
Windowsの場合スタート画面にUbuntuが追加されているので、そこから起動してください。
スタート画面からUbuntuを選択するか、Windows TerminalでUbuntuを選択することができます

![スタート画面から実行](image-7.png)

![Windows Terminal](image-12.png)

---

## GitHub Copilotでセットアップをサポートしてもらう
...
``` markdown
現在活動しているWSLの環境の中で下記を使用したいからセットアップしてください。Docker Desktop以外を利用する想定です。
- DevContainer
- Git
```
...
Ubuntu上で、以下のコマンドを実行して、ハンズオンで使用するソースコードをローカルに持ってきます

```bash
git clone https://github.com/Takas0522/ws-demo-beginner-and-intermediate.git
```
