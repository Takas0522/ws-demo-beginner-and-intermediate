...
cleanup() {
  ...
  docker compose -f "${REPO_ROOT}/src/auth-service/database/docker-compose.yml" down 2>/dev/null || true
  echo "全プロセスを停止しました。"
}
...
# ── データベース起動 ──────────────────────────────────────────────────────────
echo "[1/6] データベースを起動中..."
docker compose -f "${REPO_ROOT}/src/auth-service/database/docker-compose.yml" up -d
...
dotnet build "${REPO_ROOT}/src/auth-service/backend/AuthService" -v:minimal 2>&1 | grep -E "^Build|error TS|error CS" || true
...
cd "${REPO_ROOT}/src/auth-service/backend/AuthService"
